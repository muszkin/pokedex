...
func Catch(pokemon string) (Pokemon, error) {
	var pokemonResults Pokemon
	url := fmt.Sprintf("%s%s", pokemonEndpoint, pokemon)
	data, inCache := cache.Get(url)
	if inCache {
		if err := json.Unmarshal(data, &pokemonResults); err != nil {
	...
	if err := json.Unmarshal(body, &pokemonResults); err != nil {
		return pokemonResults, err
	}
	defer cache.Add(url, body)
	return pokemonResults, nil
